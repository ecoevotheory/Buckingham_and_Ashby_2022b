/***********************************************************************************************************
 * [t,S,I,R,EQFLAG] = Simulation_aTradeoff_function(t_max,a0,c1a,c2a,beta,b,bR,zeta_current,q,f,alpha,gamma,eqtol,init_pop,strain_total)
 ***********************************************************************************************************/

/* Compile in Matlab using mex Simulation_aTradeoff_function.c */
...
void dynamic(double *S, double *I, double *R, double *DSDT, double *DIDT, double *DRDT, double *zeta, struct PARAM *p){
    ...
    /* ODEs */
    for(i=0;i<p->strain_total;i++){
	DSDT[i] = (p->a0*(1-((p->c1a*(1-exp(-p->c2a*zeta[i])))/(1-exp(-p->c2a)))))*(1 - p->q*N)*(S[i] + p->f*I[i] +R[i]) - p->beta*S[i]*allinfecteds + p->gamma*I[i] - p->b*S[i] - zeta[i]*S[i];
