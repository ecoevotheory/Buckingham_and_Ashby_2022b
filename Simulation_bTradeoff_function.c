/***********************************************************************************************************
 * [t,S,I,R,EQFLAG] = Simulation_bTradeoff_function(t_max,a,beta,b0,aR,c1b,c2b,zeta_current,q,f,alpha,gamma,eqtol,init_pop,strain_total)
 ***********************************************************************************************************/

/* Compile in Matlab using mex Simulation_bTradeoff_function.c */
...
void dynamic(double *S, double *I, double *R, double *DSDT, double *DIDT, double *DRDT, double *zeta, struct PARAM *p){
	...
	DRDT[i] = - (p->b0*(1+((p->c1b*(1-exp(-p->c2b*zeta[i])))/(1-exp(-p->c2b)))))*R[i] + zeta[i]*S[i];
    }
}
