/***********************************************************************************************************
 * [t,S,I,R,EQFLAG] = Simulation_constantcost_function(t_max,a,beta,b,aR,bR,zeta_current,q,f,alpha,gamma,eqtol,init_pop,strain_total)
 ***********************************************************************************************************/

/* Compile in Matlab using mex Simulation_constantcost_function.c */
...
void dynamic(double *S, double *I, double *R, double *DSDT, double *DIDT, double *DRDT, double *zeta, struct PARAM *p){
    ...
    /* ODEs */
    for(i=0;i<p->strain_total;i++){
	DSDT[i] = p->a*(1 - p->q*N)*(S[i] + p->f*I[i]) + p->aR*(1-p->q*N)*R[i] - p->beta*S[i]*allinfecteds + p->gamma*I[i] - p->b*S[i] - zeta[i]*S[i];
