...
% stability of singular strategies (in the case where costs are paid
% throughout the host's lifetime). 
...
% Format data:
for i=1:length(varvec)
    if min([CSS(i,2),BP(i,2),rep(i,2),other(i,2),GOE(i,2)])==min(rep(i,2),GOE(i,2)) && isnan(CSS(i,2))
        CSS(i,2)=0;
        CSS(i,1)=i;
    end
end
...
hold on
plot(xothervec,yothervec,'x','color',midblue,'markersize',8,'linewidth',2)
set(gca,'xtick',[0,5],'fontsize',14)
set(gca,'ytick',[0,1],'fontsize',14)
ylim([0,ymax])
text(0.25,0.9,'C','fontsize',16)
xlabel(xaxislabel,'interpreter','latex');
axis square
...
yrepvec1(isnan(yrepvec1))=[];
yrepvec2(isnan(yrepvec2))=[];
xCSSvec1=NaN(length(yvec),1);
xCSSvec2=NaN(length(yvec),1);
yCSSvec1=NaN(length(yvec),1);
yCSSvec2=NaN(length(yvec),1);
for i=1:length(yvec)
    if xvec(i)>1.25
        yCSSvec1(i)=yvec(i);
        xCSSvec1(i)=xvec(i);
...
xCSSvec1(isnan(xCSSvec1))=[];
xCSSvec2(isnan(xCSSvec2))=[];
yCSSvec1(isnan(yCSSvec1))=[];
yCSSvec2(isnan(yCSSvec2))=[];
...
plot(xrepvec2,yrepvec2,':','color',orange,'markersize',8,'linewidth',3)
hold on
plot(xCSSvec1,yCSSvec1,'-','color',green,'markersize',8,'linewidth',3)
hold on
plot(xCSSvec2,yCSSvec2,'-','color',green,'markersize',8,'linewidth',3)
hold on
plot(xothervec,yothervec,'x','color',midblue,'markersize',8,'linewidth',2)
set(gca,'xtick',[0,2],'fontsize',14)
set(gca,'ytick',[0,1],'fontsize',14)
ylim([0,ymax])
text(0.1,0.9,'F','fontsize',16)
xlabel(xaxislabel,'interpreter','latex');
axis square
